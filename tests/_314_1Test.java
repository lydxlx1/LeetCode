...
public class _314_1Test {
    ...
    public void test() {
        ...
        List<?> actual = sol.verticalOrder(Deserializer.deserialize(new Integer[]{3, 9, 20, null, null, 15, 7}));
        List<?> expected = Arrays.asList(Arrays.asList(9), Arrays.asList(3, 15), Arrays.asList(20), Arrays.asList(7));
        assertEquals(expected, actual);

        actual = sol.verticalOrder(Deserializer.deserialize(new Integer[]{3, 9, 8, 4, 0, 1, 7}));
        expected = Arrays.asList(Arrays.asList(4), Arrays.asList(9), Arrays.asList(3, 0, 1), Arrays.asList(8), Arrays.asList(7));
        assertEquals(expected, actual);

        actual = sol.verticalOrder(Deserializer.deserialize(new Integer[]{3, 9, 8, 4, 0, 1, 7, null, null, null, 2, 5}));
